...
/* The `tease` paragraph below the H1 is the single source-of-truth lesson
   ...
   overview (`/chapter/2`) card. See `course-nav.ts` sub `tease`. */
    ...
    <section className="relative min-h-screen flex flex-col justify-center px-6 md:px-10">
      ...
      <div className="relative max-w-[1100px] mx-auto w-full">
        ...
        <h1
          ...
          className="title-gradient font-serif font-medium tracking-tight leading-[1.04]"
        >
          <span className="block" style={{ fontSize: "clamp(2.1rem, 5.4vw, 4.6rem)" }}>
            Formation <span className="italic font-light text-white/70">&</span>
          </span>
          <span className="block" style={{ fontSize: "clamp(2.1rem, 5.4vw, 4.6rem)" }}>
            Evolution <span className="italic font-light text-white/70">of</span>
          </span>
          <span className="block" style={{ fontSize: "clamp(2.1rem, 5.4vw, 4.6rem)" }}>
            Galaxies
          </span>
      ...
      <div className="hidden lg:flex absolute right-10 bottom-10 flex-col items-end gap-1 font-mono text-[10px] tracking-[0.2em] uppercase text-white/40">
        <div>filaments · clusters · voids</div>
        <div className="text-plasma/60">+ mergers: starburst · elliptical · spiral</div>
      </div>
    </section>
  );
}
